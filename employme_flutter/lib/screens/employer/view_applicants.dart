...
class _ViewApplicantsState extends State<ViewApplicants> {
  ...
  Widget _header() => Container(color: AppColors.card, padding: const EdgeInsets.fromLTRB(20, 16, 20, 16), child: Column(children: [
    ...
    SizedBox(height: 32, child: ListView(scrollDirection: Axis.horizontal, children: ['All (47)', 'New (12)', 'Shortlisted (5)', 'Invited (3)', 'Contacted (8)', 'Hired (2)'].map((t) {
  ...
  Widget _applicantCard(BuildContext context, AppState state, Map<String, dynamic> a) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        color: (isShortlisted || isInvited) ? AppColors.navyLighter.withOpacity(0.5) : AppColors.card,
        borderRadius: BorderRadius.circular(16),
        boxShadow: (isShortlisted || isInvited) ? null : [BoxShadow(color: Colors.black.withOpacity(0.08), blurRadius: 12)],
        border: (isShortlisted || isInvited) ? Border.all(color: AppColors.border) : null,
      ),
      child: Stack(
        children: [
          Padding(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Container(width: 52, height: 52, decoration: BoxDecoration(color: isShortlisted ? AppColors.primaryLight : AppColors.primary.withOpacity(0.1), shape: BoxShape.circle), alignment: Alignment.center, child: Text(a['initial'], style: TextStyle(fontSize: 22, fontWeight: FontWeight.w700, color: isShortlisted ? AppColors.primary : AppColors.primaryDark))),
                ...
                Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  ...
                  const SizedBox(height: 6), Wrap(spacing: 4, children: (a['skills'] as List<String>).map((s) => Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 2), decoration: BoxDecoration(color: (isShortlisted || isInvited) ? AppColors.border : AppColors.primaryLight, borderRadius: BorderRadius.circular(8)),
                    child: Row(mainAxisSize: MainAxisSize.min, children: [Icon(Icons.check_circle, size: 10, color: (isShortlisted || isInvited) ? AppColors.caption : AppColors.primary), const SizedBox(width: 4), Text(s, style: TextStyle(fontSize: 10, fontWeight: FontWeight.w600, color: (isShortlisted || isInvited) ? AppColors.caption : AppColors.primary))]))).toList()),
                  const SizedBox(height: 4), Row(children: [Icon(Icons.directions_walk, size: 14, color: (isShortlisted || isInvited) ? AppColors.caption : AppColors.primary), const SizedBox(width: 4), Text('${a['distance']}', style: TextStyle(fontSize: 12, color: (isShortlisted || isInvited) ? AppColors.caption : AppColors.primary, fontWeight: FontWeight.w600))]),
                  Row(children: [
              ...
              Row(children: [
                ...
                Expanded(flex: 3, child: SizedBox(height: 40, child: ElevatedButton(
                  onPressed: (isShortlisted || isInvited) ? null : () => state.inviteApplicant(a['name']), 
                  style: ElevatedButton.styleFrom(backgroundColor: (isShortlisted || isInvited) ? AppColors.border : AppColors.primary, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10))), 
